...
function foodCostBg(pct: number) {
  if (pct < 28) return 'bg-blue-50 text-blue-700'
...
type TabId = 'latest' | 'category' | 'margin'
...
export default function Compare() {
  ...
  // Group filtered items by category (for By Category tab)
  ...
  if (isLoading) return <div className="flex items-center justify-center h-full text-gray-400 text-sm">Loading prices…</div>
  if (isError || !data) return <div className="flex items-center justify-center h-full text-red-500 text-sm">Failed to load. Is the backend running?</div>
  if (data.items.length === 0 && data.distributors.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-3 text-gray-400">
        ...
        <a href="/upload" className="mt-2 px-5 py-2 bg-blue-800 text-white text-sm font-semibold rounded-full hover:bg-blue-700 transition-colors">Go to Upload</a>
      </div>
    )
  }
  ...
  const TABS: { id: TabId; label: string }[] = [
    ...
    { id: 'margin', label: 'Margin View' },
  ]

  // ── Shared table header for Latest + By Category ──
  const priceTableHead = (
    <thead className="sticky top-0 bg-white/95 backdrop-blur-sm z-10">
      <tr className="border-b border-gray-100">
        ...
        <th className="text-left px-4 py-3 text-[10px] font-semibold text-gray-400 uppercase tracking-widest min-w-[200px]">Item</th>
        <th className="text-left px-4 py-3 text-[10px] font-semibold text-gray-400 uppercase tracking-widest w-32">Category</th>
        {data.distributors.map(d => (
          <th key={d.id} className="text-center px-4 py-3 text-[10px] font-semibold text-gray-400 uppercase tracking-widest min-w-[120px]">
        ...
        <th className="text-center px-4 py-3 text-[10px] font-semibold text-gray-400 uppercase tracking-widest w-24">Best Price</th>
        <th className="text-center px-4 py-3 text-[10px] font-semibold text-gray-400 uppercase tracking-widest w-24">Menu $</th>
        <th className="text-center px-4 py-3 text-[10px] font-semibold text-gray-400 uppercase tracking-widest w-24">Food Cost%</th>
      </tr>
    </thead>
  )

  // ── Shared row renderer for Latest + By Category ──
  function renderItemRow(item: CompareItem) {
    const isChecked = selected[item.canonical_id] !== undefined
    const bestPrice = item.best_distributor_id ? item.prices[item.best_distributor_id]?.price : undefined
    const foodCostPct = item.menu_price && bestPrice ? (bestPrice / item.menu_price * 100) : null
    const hasSpike = Object.values(item.prices).some(p => p.change_pct != null && p.change_pct > 10)

    return (
      <tr
        key={item.canonical_id}
        onClick={() => toggleItem(item)}
        className={`border-b border-gray-50 cursor-pointer transition-colors ${isChecked ? 'bg-blue-50/60' : 'hover:bg-gray-50/70'}`}
      >
        <td className="px-4 py-3">
          <input type="checkbox" checked={isChecked} onChange={() => {}} className="rounded pointer-events-none" />
        </td>
        <td className="px-4 py-3">
          <div className="flex items-center gap-2">
            {hasSpike && <AlertTriangle size={12} className="text-orange-400 shrink-0" />}
            <span className="font-medium text-gray-900 text-sm">{item.canonical_name}</span>
          </div>
        </td>
        ...
        {data!.distributors.map(d => {
          ...
          if (!entry) return <td key={d.id} className="px-4 py-3 text-center text-gray-200 text-sm">—</td>
          return (
            <td key={d.id} className={`px-4 py-3 text-center ${isBest ? 'bg-blue-50' : ''}`}>
              <div className={`font-semibold text-sm ${isBest ? 'text-blue-700' : 'text-gray-800'}`}>{fmt(entry.price)}</div>
              {entry.pack_size && <div className="text-[10px] text-gray-400 mt-0.5">{entry.pack_size}</div>}
              <ChangeChip pct={entry.change_pct} />
            </td>
          )
        })}
        <td className="px-4 py-3 text-center">
          {bestPrice !== undefined ? <span className="font-bold text-blue-700 text-sm">{fmt(bestPrice)}</span> : <span className="text-gray-200">—</span>}
        </td>
        ...
        <td className="px-4 py-3 text-center">
          {foodCostPct !== null ? <span className={`text-sm ${foodCostColor(foodCostPct)}`}>{foodCostPct.toFixed(0)}%</span> : <span className="text-gray-200">—</span>}
        </td>
      </tr>
    )
  }
    ...
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="bg-white border-b border-gray-100 px-4 sm:px-6 pt-5 pb-0 shrink-0">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start gap-3 sm:gap-4 mb-5">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {selectedCount > 0 && (
              <button onClick={handleBuildOrder} className="inline-flex items-center gap-1.5 px-4 py-1.5 bg-blue-800 text-white text-xs font-semibold rounded-full hover:bg-blue-700 transition-colors">
                <ShoppingCart size={12} />Build Order ({selectedCount})
              </button>
            )}
          </div>
        </div>
      ...
      {activeTab === 'latest' && (
        <div className="flex-1 overflow-auto bg-white">
          ...
          {filtered.length === 0 && <div className="text-center py-20 text-gray-300 text-sm">No items match your filter.</div>}
        </div>
      )}
      ...
      {activeTab === 'category' && (
        <div className="flex-1 overflow-auto bg-white">
          <table className="w-full text-sm border-collapse">
            ...
            <tbody>
              {byCategory.map(([cat, items]) => (
                <>
                  {/* Category header row */}
                  <tr key={`cat-${cat}`} className="bg-gray-900">
                    <td colSpan={4 + data.distributors.length} className="px-4 py-2">
                      <div className="flex items-center gap-2">
                        <span className="text-[11px] font-bold text-white uppercase tracking-widest">{cat}</span>
                        <span className="text-[10px] text-gray-400 font-normal">{items.length} item{items.length !== 1 ? 's' : ''}</span>
                      </div>
                    </td>
                  </tr>
          ...
          {byCategory.length === 0 && <div className="text-center py-20 text-gray-300 text-sm">No items match your filter.</div>}
        </div>
      )}
          ...
          <table className="w-full text-sm border-collapse">
            <thead className="sticky top-0 bg-white/95 backdrop-blur-sm z-10">
              <tr className="border-b border-gray-100">
                ...
                <th className="text-left px-4 py-3 text-[10px] font-semibold text-gray-400 uppercase tracking-widest w-32">Category</th>
                {data.distributors.map(d => (
                  <th key={d.id} colSpan={2} className="text-center px-4 py-3 text-[10px] font-semibold text-gray-400 uppercase tracking-widest border-l border-gray-100">
                ...
                <th className="text-center px-4 py-3 text-[10px] font-semibold text-gray-400 uppercase tracking-widest w-24">Menu $</th>
                <th className="text-center px-4 py-3 text-[10px] font-semibold text-gray-400 uppercase tracking-widest w-28">Best Choice</th>
              </tr>
              <tr className="border-b border-gray-100 bg-gray-50/60">
                <th colSpan={2} />
                {data.distributors.map(d => (
                  <>
                    <th key={`${d.id}-price`} className="text-center px-3 py-1.5 text-[9px] font-semibold text-gray-400 uppercase tracking-widest border-l border-gray-100 w-24">Price</th>
                    <th key={`${d.id}-fc`} className="text-center px-3 py-1.5 text-[9px] font-semibold text-gray-400 uppercase tracking-widest w-20">Food Cost%</th>
                  </>
                ))}
                <th />
                <th />
            ...
            <tbody>
              {byCategory.map(([cat, items]) => (
                <>
                  <tr key={`mcat-${cat}`} className="bg-gray-900">
                    <td colSpan={3 + data.distributors.length * 2} className="px-4 py-2">
                      <div className="flex items-center gap-2">
                        <span className="text-[11px] font-bold text-white uppercase tracking-widest">{cat}</span>
                  ...
                  {items.map(item => {
                    const bestPrice = item.best_distributor_id ? item.prices[item.best_distributor_id]?.price : undefined
                    const bestDist = data.distributors.find(d => d.id === item.best_distributor_id)

                    return (
                      <tr key={item.canonical_id} className="border-b border-gray-50 hover:bg-gray-50/70 transition-colors">
                        <td className="px-4 py-3 font-medium text-gray-900 text-sm">{item.canonical_name}</td>
                        <td className="px-4 py-3 text-[11px] text-gray-400">{item.category || '—'}</td>

                        {data.distributors.map(d => {
                          const entry: PriceEntry | undefined = item.prices[d.id]
                          const isBest = d.id === item.best_distributor_id
                          const fc = item.menu_price && entry ? (entry.price / item.menu_price * 100) : null

                          return (
                            <>
                              <td key={`${item.canonical_id}-${d.id}-price`} className={`px-3 py-3 text-center border-l border-gray-100 ${isBest ? 'bg-blue-50' : ''}`}>
                                {entry
                                  ? <span className={`font-semibold text-sm ${isBest ? 'text-blue-700' : 'text-gray-800'}`}>{fmt(entry.price)}</span>
                                  : <span className="text-gray-200">—</span>}
                              </td>
                              <td key={`${item.canonical_id}-${d.id}-fc`} className={`px-3 py-3 text-center ${isBest ? 'bg-blue-50' : ''}`}>
                                {fc !== null
                                  ? <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${foodCostBg(fc)}`}>{fc.toFixed(0)}%</span>
                        ...
                        <td className="px-4 py-3 text-center text-gray-500 text-sm">
                          {item.menu_price ? fmt(item.menu_price) : <span className="text-gray-300 text-xs">set price</span>}
                        </td>
                        <td className="px-4 py-3 text-center">
                          {bestDist && bestPrice !== undefined ? (
                            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-blue-50 border border-blue-100">
                              <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: bestDist.color }} />
                              <span className="text-[11px] font-semibold text-blue-700">{bestDist.name}</span>
                              <span className="text-[11px] text-blue-500">{fmt(bestPrice)}</span>
                            </div>
                          ) : <span className="text-gray-200">—</span>}
                        </td>
                      </tr>
      ...
      {showOrderModal && (
        <div className="fixed inset-0 bg-black/25 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl shadow-2xl border border-gray-100 p-6 w-96">
            ...
            <p className="text-xs text-gray-400 mb-5">{selectedCount} items will be grouped by cheapest supplier.</p>
            <div className="mb-5">
              <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider block mb-1.5">Order name (optional)</label>
              <input className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-gray-400" value={orderName} onChange={e => setOrderName(e.target.value)} placeholder={`Order — ${data.week_date}`} />
            </div>
            <div className="flex gap-3">
              <button className="flex-1 px-4 py-2.5 border border-gray-200 rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors" onClick={() => setShowOrderModal(false)}>Cancel</button>
              <button className="flex-1 px-4 py-2.5 bg-blue-800 text-white rounded-xl text-sm font-semibold hover:bg-blue-700 transition-colors disabled:opacity-50" onClick={handleConfirmOrder} disabled={createOrderMutation.isPending}>
