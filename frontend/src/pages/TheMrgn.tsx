...
const SUBSYSTEMS = [
  {
    ...
    description:
      'A shared invoice and pricing database giving every enrolled subscriber real-time benchmark data â€” what other operators are paying for common ingredients, from which vendors, at what volume. Fully anonymized at the individual restaurant level.',
    bullets: [
  ...
  },
  {
    ...
    bullets: [
      ...
      'Anonymized by default â€” opt-in to show your name',
    ],
  },
]
...
export default function TheMrgn() {
      ...
      <div style={{ background: 'linear-gradient(135deg, #071733 0%, #0b1e42 60%, #0f2d5e 100%)' }}>
        <div className="px-5 sm:px-10 pt-8 sm:pt-12 pb-10 sm:pb-14">
          ...
          <p className="text-sm max-w-xl leading-relaxed mb-8" style={{ color: 'rgba(147,197,253,0.65)' }}>
            The Mrgn gives restaurant operators access to community price benchmarks, group buying power,
            and a shared intelligence feed â€” three tools no standalone platform currently combines.
            ...
            <div key={tier.id} className="rounded-2xl bg-white border border-gray-100 p-7 flex flex-col gap-5"
              ...
              <div className="flex items-start justify-between">
                ...
                <div className="text-right">
                  <div className="text-3xl font-black" style={{ color: tier.color }}>{tier.monthly}</div>
      ...
      <div className="px-5 sm:px-10 pb-10 sm:pb-12">
        <div className="rounded-2xl bg-white border border-gray-100 p-7" style={{ boxShadow: '0 1px 8px rgba(0,0,0,0.05)' }}>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3 text-sm">
            {[
              { step: '01', title: 'Upload your invoices', desc: 'Submit your real invoices for any ingredient you purchase. Your data is anonymized â€” no one sees your raw invoice.' },
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden">
            ...
            {submitted ? (
              <div className="px-7 py-10 flex flex-col items-center gap-4 text-center">
                <div className="w-14 h-14 rounded-full flex items-center justify-center" style={{ background: 'rgba(37,99,235,0.1)' }}>
                ...
                <div>
                  ...
                  <div className="text-sm text-gray-500 leading-relaxed">
                    We've received your request for The Mrgn {form.tier === 'enterprise' ? 'Enterprise' : 'Professional'} tier.
                    The 1LT team will contact you at <strong>{form.email}</strong> within 1 business day.
                  </div>
                </div>
                <button onClick={() => setShowModal(false)} className="mt-2 px-6 py-2.5 rounded-xl font-bold text-sm" style={{ background: '#2563eb', color: '#fff' }}>Done</button>
              </div>
            ) : (
              <form onSubmit={submit} className="px-7 py-6 flex flex-col gap-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="col-span-2">
                    ...
                    <input required className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-blue-500 transition-colors" value={form.restaurant_name} onChange={e => field('restaurant_name', e.target.value)} placeholder="e.g. Olive & Oregano" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 mb-1">Your Name *</label>
                    <input required className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-blue-500 transition-colors" value={form.contact_name} onChange={e => field('contact_name', e.target.value)} placeholder="Full name" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 mb-1">Email *</label>
                    <input required type="email" className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-blue-500 transition-colors" value={form.email} onChange={e => field('email', e.target.value)} placeholder="you@restaurant.com" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-gray-500 mb-1">Tier *</label>
                    <select required className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-blue-500 transition-colors bg-white" value={form.tier} onChange={e => field('tier', e.target.value)}>
                  ...
                  <div>
                    ...
                    <input type="number" min="1" className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-blue-500 transition-colors" value={form.locations} onChange={e => field('locations', e.target.value)} />
                  </div>
                  <div className="col-span-2">
                    <label className="block text-xs font-semibold text-gray-500 mb-1">Notes (optional)</label>
                    <textarea className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-blue-500 transition-colors resize-none" rows={2} value={form.notes} onChange={e => field('notes', e.target.value)} placeholder="Questions, special requirements, or contextâ€¦" />
                  </div>
                </div>
                <button type="submit" disabled={submitting} className="w-full py-3 rounded-xl font-bold text-sm transition-all disabled:opacity-50 flex items-center justify-center gap-2" style={{ background: '#1e40af', color: '#fff' }}>
                  {submitting ? 'Submittingâ€¦' : <>Request Access <ChevronRight size={15} /></>}
                </button>
                <p className="text-center text-xs text-gray-400">No payment required now. The 1LT team will contact you to finalize your subscription.</p>
              </form>
            )}
          </div>
        </div>
